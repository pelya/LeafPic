    ...
    while (s1_pos < s1.length() || s2_pos < s2.length())
      {
        ...
        while (s1.codePointAt(s1_pos) == '0')
          s1_pos++;
        while (s2.codePointAt(s2_pos) == '0')
          s2_pos++;
        while (c_isdigit (s1.codePointAt(s1_pos)) && c_isdigit (s2.codePointAt(s2_pos)))
          {
            if (first_diff == 0)
        ...
        if (c_isdigit (s1.codePointAt(s1_pos)))
          return 1;
        if (c_isdigit (s2.codePointAt(s2_pos)))
          return -1;
        if (first_diff != 0)
          return first_diff;
      }
